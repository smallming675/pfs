...
char *file_path_join(const file_path *fp) {
  ...
  log_msg(LOG_DEBUG, "file_path_join: Joined %zu parts into path '%s'.", fp->count, out);
  return out;
}
...
bool is_valid_dir(const fs *fs, const char *dir_name, uint32_t dir_node_id) {
  log_msg(LOG_INFO, "is_valid_dir: Checking if '%s' is a valid directory under node %u.",
          dir_name, dir_node_id);
  uint32_t id = find_dir_node(fs, dir_name, dir_node_id);
  if (id == NULL_NODE_ID) {
    log_msg(LOG_ERROR, "is_valid_dir: Directory '%s' not found under node %u.", dir_name,
            dir_node_id);
    return false;
  }
  if (fs->table[id].status != NODE_DIR_ENTRY) {
    log_msg(LOG_ERROR, "is_valid_dir: Node %u for '%s' is not a directory.", id, dir_name);
    return false;
  }
  log_msg(LOG_INFO, "is_valid_dir: Directory '%s' resolved to node %u.", dir_name, id);
  return true;
}

bool is_valid_path(const fs *fs, const char *name, uint32_t dir_node_id) {
  uint32_t id = find_dir_node(fs, name, dir_node_id);
  log_msg(LOG_DEBUG, "is_valid_path: Validating path '%s' under node %u: %s", name,
          dir_node_id, id == NULL_NODE_ID ? "NOT FOUND" : "FOUND");
...
bool has_name_conflict(const fs *fs, uint32_t dir_node_id, const char *name) {
  if (fs->table[dir_node_id].status != NODE_DIR_ENTRY) {
    log_msg(LOG_ERROR,
            "has_name_conflict: Node %u is not a directory, cannot check for conflicts.",
            dir_node_id);
    return true;
  }
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (fs->table[id].status == NODE_DIR_ENTRY &&
      ...
      log_msg(LOG_DEBUG, "has_name_conflict: Conflict: directory '%s' already exists in %u.", name,
              dir_node_id);
      return true;
    }
    if ((fs->table[id].status == NODE_SINGLE_NODE_FILE ||
      ...
      log_msg(LOG_DEBUG, "has_name_conflict: Conflict: file '%s' already exists in %u.", name,
              dir_node_id);
      return true;
    }
  }
...
int create_file_at_dir(fs *fs, uint32_t dir_node_id, const char *file_name,
  ...
  log_msg(LOG_INFO, "create_file_at_dir: Creating file '%s' in directory %u.", file_name,
          dir_node_id);

  if (fs->table[dir_node_id].status != NODE_DIR_ENTRY) {
    log_msg(LOG_ERROR, "create_file_at_dir: Node %u is not a directory.", dir_node_id);
    return NULL_NODE_ID;
  }
  if (has_name_conflict(fs, dir_node_id, file_name)) {
    log_msg(LOG_ERROR,
            "create_file_at_dir: Cannot create file '%s': name conflict in directory %u.",
            file_name, dir_node_id);
    return NULL_NODE_ID;
  }
  uint32_t node_id = fs_allocate_node(fs);
  if (node_id == NULL_NODE_ID) {
    log_msg(LOG_ERROR, "create_file_at_dir: No free node available to create file '%s'.",
            file_name);
    return NULL_NODE_ID;
  }
  int ok = write_file(fs, file_name, dir_node_id, data, size);
  if (!ok) {
    log_msg(LOG_ERROR, "create_file_at_dir: Failed to initialize file '%s' in directory %u.",
            file_name, dir_node_id);
    return NULL_NODE_ID;
  }
  if (insert_file_to_dir(fs, dir_node_id, node_id) != 0) {
    log_msg(LOG_ERROR, "create_file_at_dir: Failed to insert file node %u into directory %u.",
            node_id, dir_node_id);
    return NULL_NODE_ID;
  }

  log_msg(LOG_INFO, "create_file_at_dir: File '%s' created as node %u in directory %u.", file_name,
          node_id, dir_node_id);
  log_msg(LOG_DEBUG, "create_file_at_dir: Directory %u now has %u entries.", dir_node_id,
...
int insert_file_to_dir(fs *fs, uint32_t dir_node_id, uint32_t file_node_id) {
  log_msg(LOG_INFO, "insert_file_to_dir: Inserting node %u into directory %u...", file_node_id,
          dir_node_id);
  if (fs->table[dir_node_id].status != NODE_DIR_ENTRY) {
    log_msg(LOG_ERROR, "insert_file_to_dir: Node %u is not a directory.", dir_node_id);
    return 1;
  }
  ...
  if (new_name && has_name_conflict(fs, dir_node_id, new_name)) {
    log_msg(LOG_ERROR,
            "insert_file_to_dir: Cannot insert node %u: name '%s' already exists in directory %u.",
...
int remove_file_from_dir(fs *fs, uint32_t dir_node_id, uint32_t file_node_id) {
  log_msg(LOG_INFO, "remove_file_from_dir: Removing node %u from directory %u.", file_node_id,
          dir_node_id);
  if (fs->table[dir_node_id].status != NODE_DIR_ENTRY) {
    log_msg(LOG_ERROR, "remove_file_from_dir: Node %u is not a directory.", dir_node_id);
    return 1;
  }
  uint32_t count = fs->table[dir_node_id].data.dir_entry.entry_count;
  for (uint32_t i = 0; i < count; i++) {
  ...
  log_msg(LOG_ERROR, "remove_file_from_dir: Node %u not found in directory %u.", file_node_id,
          dir_node_id);
  return 0;
}

int create_dir(fs *fs, uint32_t parent_id, const char *name) {
  log_msg(LOG_INFO, "create_dir: Creating directory '%s' under parent %u.", name,
          parent_id);
  if (fs->table[parent_id].status != NODE_DIR_ENTRY) {
    log_msg(LOG_ERROR, "create_dir: Parent node %u is not a directory.", parent_id);
    return NULL_NODE_ID;
  }
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (fs->table[id].status == NODE_DIR_ENTRY &&
        strcmp(fs->table[id].data.dir_entry.dir_name, name) == 0) {
      log_msg(LOG_ERROR, "create_dir: Directory '%s' already exists under parent %u.", name,
              parent_id);
      return NULL_NODE_ID;
    }
    if ((fs->table[id].status == NODE_SINGLE_NODE_FILE ||
         fs->table[id].status == NODE_FILE_START) &&
        strcmp(fs->table[id].data.header_file.file_name, name) == 0) {
      log_msg(LOG_ERROR, "create_dir: File '%s' already exists under parent %u.", name,
              parent_id);
      return NULL_NODE_ID;
    }
  }
  ...
  uint32_t node_id = fs_allocate_node(fs);
  if (node_id == NULL_NODE_ID) {
    log_msg(LOG_ERROR, "create_dir: No free node available to create directory '%s'.",
            name);
    return NULL_NODE_ID;
  }
...
uint8_t *read_from_path(fs *fs, const char *path, int meta_only,
  ...
  if (!buf) {
    log_msg(LOG_ERROR, "read_from_path: Failed to read file '%s' in directory %u.", rp.filename,
            rp.dir_id);
  } else {
...
void free_resolved_path(resolved_path *rp) {
  if (rp && rp->filename) {
    log_msg(LOG_DEBUG, "free_resolved_path: Freeing resolved filename '%s'.", rp->filename);
    free(rp->filename);
    rp->filename = NULL;
  }
}
...
resolved_path resolve_path(fs *fs, const char *path, uint32_t start_dir) {
  log_msg(LOG_INFO, "resolve_path: Resolving path '%s' from start dir %u.", path, start_dir);
  resolved_path rp = {NULL_NODE_ID, NULL};
  file_path fp = file_path_split(path);
  if (fp.count == 0) {
    log_msg(LOG_ERROR, "resolve_path: Path '%s' split into zero components.", path);
    file_path_free(&fp);
    return rp;
  }
  ...
  for (size_t i = 0; i + 1 < fp.count; i++) {
    ...
    if (next_id == NULL_NODE_ID) {
      log_msg(LOG_ERROR, "resolve_path: Directory '%s' not found under node %u.", fp.parts[i],
              rp.dir_id);
      file_path_free(&fp);
      rp.dir_id = NULL_NODE_ID;
      return rp;
    }
    log_msg(LOG_DEBUG, "resolve_path: Component '%s' resolved to node %u.", fp.parts[i],
            next_id);
    rp.dir_id = next_id;
  }
  ...
  if (last[0] != '\0') {
    ...
    log_msg(LOG_INFO, "resolve_path: Final component '%s' resolved in directory %u.",
            rp.filename, rp.dir_id);
  } else {
    log_msg(LOG_INFO, "resolve_path: Path ended with '/', resolved directory id %u.",
            rp.dir_id);
  }
